...
if (
    which("ar") is None
    and not os.path.isfile("./ar")
    and not os.path.isfile("./ar.exe")
):
    raise Exception(
...
if (
    which("openssl") is None
    and not os.path.isfile("./openssl")
    and not os.path.isfile("./openssl.exe")
):
    raise Exception(
        "You need to have openssl in PATH, installable from https://www.openssl.org/"
    )
...
for i in range(0, len(pems)):
    ...
    ssl = Popen(
        ["openssl", "x509", "-inform", "PEM", "-outform", "DER", "-out", certName],
        shell=False,
        stdin=PIPE,
    )
    pipe = ssl.stdin
